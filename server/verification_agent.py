...
class VerificationAgent:
    ...
    def generate_prompt(self, answer: str, context: str) -> str:
        return f"""You are an AI assistant designed to verify the accuracy and relevance of answers based on the provided context.
                Instructions:
    ...
    def parse_verification_response(self, response_text: str) -> Dict:
        try:
                    ...
                    if key in {"Supported", "Unsupported claims", "Contradictions", "Relevant", "Additional details"}:
                        if key in {"Unsupported claims", "Contradictions"}:
                            if value.startswith('[') and value.endswith(']'):
                                ...
                                verification[key] = [item.strip().strip('"').strip("'") for item in items if item.strip()]
                            else:
                                verification[key] = []
            ...
            return {"Supported": "NO", "Unsupported Claims": [], "Contradictions": [], "Relevant": "NO", "Additional Details": ""}
    ...
    def check(self, answer: str, documents: List[Document]) -> Dict:
        ...
        try:
            ...
            fallback = {"Supported": "NO", "Unsupported Claims": [], "Contradictions": [], "Relevant": "NO", "Additional Details": "Invalid response structure from the model."}
            return {"verification_report": self.format_verification_report(fallback), "context_used": context}

        sanitized_response = self.sanitize_response(llm_response) if llm_response else ""
        if not sanitized_response:
            print("LLM returned an empty response.")
            verification_report = {"Supported": "NO", "Unsupported Claims": [], "Contradictions": [], "Relevant": "NO", "Additional Details": "Empty response from the model."}
        else:
            ...
            if verification_report is None:
                ...
                verification_report = {"Supported": "NO", "Unsupported Claims": [], "Contradictions": [], "Relevant": "NO", "Additional Details": "Failed to parse the model's response."}

        verification_report_formatted = self.format_verification_report(verification_report)
        print(f"Verification report:\n{verification_report_formatted}")
        return {"verification_report": verification_report_formatted, "context_used": context}
